...
- **DB name aislado por app (CRITICO)**: el cluster es compartido, asi que CADA app debe usar su propia database, NUNCA un nombre fijo como `'app'` ni el default del driver (`test`) — sino colisionan todas. `coolify_set_mongodb_env` setea `MONGODB_DB_NAME=papolo_<short>` automaticamente; tu codigo DEBE leerlo y pasarlo al cliente. El connection string NO incluye el nombre de DB, asi que si no lo pasas explicito, el driver cae en su default (`test` en Mongoose, `test` en el driver nativo) y rompe el aislamiento. Patrones por cliente:
  - **Mongoose** (lo mas comun en SvelteKit): `mongoose.connect(process.env.MONGODB_URI, { dbName: process.env.MONGODB_DB_NAME ?? 'app' })`. Sin `dbName`, Mongoose usa `test` e ignora `MONGODB_DB_NAME`.
  - **Driver nativo Node**: `client.db(process.env.MONGODB_DB_NAME ?? 'app')`.
  - **pymongo/motor**: `client[os.environ.get("MONGODB_DB_NAME", "app")]`.

  Esa DB aislada es ademas la **test DB** que se siembra con mock data para los smoke tests. Verificalo: tras el seed, el smoke test debe ver datos; si el dashboard sale vacio aunque sembraste, probablemente el seed escribio en `papolo_<short>` pero la app lee de `test` (o viceversa) — revisá que TODO el codigo (db.ts, seed, models) use el mismo `MONGODB_DB_NAME`.
