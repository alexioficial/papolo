...
- **Testeo con `agent-browser` — LOCAL primero (ahorra deploys)**: cada deploy cuesta build+push+espera. NO itera testeando contra la preview (asi se gastan 9 deploys). El flujo pro: levanta la app LOCAL en el VPS (Mongo en docker local + `node build`), sembrala y corre el smoke test contra `localhost:3000`. Arregla TODOS los bugs ahi (segundos, cero deploys). Recien cuando el smoke LOCAL pasa, deployas UNA vez y haces un solo smoke de confirmacion contra la preview. Solo con PASS reportas el URL. Para APIs puras sin frontend, basta `curl /api/health`. (La skill `agent-browser` tiene el procedimiento exacto.)
