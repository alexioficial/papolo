...
- FASE 2 (en PARALELO): recien con la base lista, spawnea VARIOS spawn_subagent en la MISMA respuesta, uno por modulo de dominio INDEPENDIENTE (ej. un subagente para las rutas de auth, otro para el CRUD de citas, otro para el dashboard, otro para gestion de servicios/profesionales). Corren a la vez — ahi esta el ahorro real de tiempo. Para el frontend SvelteKit esto significa usar VARIOS `sveltekit-expert` en paralelo (no uno solo secuencial): uno por route-group. No dupliques el nombre pensando que hace falta un agente distinto — spawnea el MISMO `sveltekit-expert` N veces, cada instancia con su carpeta exclusiva; el motor las corre en paralelo y las distingue solas.
